...
AileEx の `-a` / `-w` オプションに `-t`（形式）/ `-m`（方式）/ `-l`（レベル）を追加し、
圧縮設定のコマンドライン制御を可能にする。
...
ダイアログスキップは `-a`/`-w` モード（`SW_HIDE`）でのみ行う。
...
// -a/-w モードのみ渡す（SW_HIDE → ダイアログスキップ判定の起点）
...
// ダイアログスキップは -a/-w モード（SW_HIDE）かつ -t 指定時のみ
...
- [ ] `main.cpp` — `-t` / `-m` / `-l` パース追加
- [ ] `App.h` — `RunCompressMode` / `RunCompressEachMode` シグネチャ変更
- [ ] `App.cpp` — `ApplyMethodOverride()` ヘルパー実装
- [ ] `App.cpp` — `RunCompressMode` オーバーライド適用 + ダイアログスキップロジック
- [ ] `App.cpp` — `RunCompressEachMode` 同上
- [ ] テスト
